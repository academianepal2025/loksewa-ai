  ...
  return (
    <div className="bg-surface border border-border-subtle rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleAddClick} className="flex items-center gap-2 px-4 py-2 bg-primary text-accent rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 shadow-lg shadow-primary/10">
        ...
        <div className="space-y-3">
          {exams.map(exam => (
            <div key={exam.id} className="bg-background border border-border-subtle rounded-xl p-4 sm:p-5">
              {editingId === exam.id ? (
                <div className="space-y-3">
                  <input value={editForm.exam_name || ''} onChange={e => setEditForm(f => ({...f, exam_name: e.target.value}))} className="w-full bg-surface border border-border-subtle rounded-xl px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-foreground outline-none focus:border-accent/50 shadow-sm" />
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleEdit} disabled={editSaving} className="px-4 py-2 bg-primary text-accent rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 disabled:opacity-40 shadow-lg shadow-primary/10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[8px] font-black px-2 py-0.5 rounded-md bg-primary text-accent uppercase tracking-widest border border-accent/20 shadow-sm">{exam.exam_category}</span>
                      <span className={`text-[8px] font-black px-2 py-0.5 rounded-md uppercase tracking-widest shadow-sm border ${exam.status === 'active' ? 'bg-accent/10 text-accent border-accent/20' : 'bg-background text-subtle border-border-subtle'}`}>{exam.status}</span>
                    </div>
          ...
          <div className="bg-surface border border-border-subtle rounded-2xl p-8 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="relative">
                <label className="text-[10px] font-bold text-subtle mb-1.5 block ml-1">Category</label>
                <select value={addForm.exam_category} onChange={e => setAddForm(f => ({...f, exam_category: e.target.value}))} className="w-full appearance-none bg-background border border-border-subtle rounded-xl px-4 py-3 text-sm font-semibold outline-none focus:border-accent/50 shadow-sm">
                  <option value="">Select Category...</option>
                  {EXAM_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <ChevronDown className="absolute right-4 top-9 h-4 w-4 text-accent pointer-events-none" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-subtle mb-1.5 block ml-1">Exam Name</label>
                <input value={addForm.exam_name} onChange={e => setAddForm(f => ({...f, exam_name: e.target.value}))} className="w-full bg-background border border-border-subtle rounded-xl px-4 py-3 text-sm font-semibold outline-none focus:border-accent/50 shadow-sm placeholder:text-subtle/30" placeholder="e.g. Kharidar 2081" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-subtle mb-1.5 block ml-1">Study Duration</label>
                <div className="grid grid-cols-4 gap-2">
                  {DURATION_PRESETS.map(d => (
                    <button key={d} type="button" onClick={() => setAddForm(f => ({...f, study_days: d, custom_days: ''}))} className={`py-2.5 rounded-xl text-xs font-bold border transition-all shadow-sm ${addForm.study_days === d && !addForm.custom_days ? 'border-accent bg-accent/5 text-accent' : 'bg-background border-border-subtle text-subtle'}`}>{d} Days</button>
                  ))}
                </div>
              </div>
              <div className="flex justify-between items-center">
                <label className="text-[10px] font-bold text-subtle ml-1">Daily Hours: <span className="text-accent">{addForm.daily_study_hours} Hours</span></label>
              </div>
              <input type="range" min="1" max="14" value={addForm.daily_study_hours} onChange={e => setAddForm(f => ({...f, daily_study_hours: parseInt(e.target.value)}))} className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-accent bg-background border border-border-subtle" />
              <button onClick={handleAdd} disabled={addSaving} className="w-full py-3.5 bg-primary text-accent rounded-xl text-xs font-bold hover:opacity-90 disabled:opacity-40 flex items-center justify-center gap-2 shadow-lg shadow-primary/10">
                {addSaving && <Loader2 className="h-3.5 w-3.5 animate-spin" />} Initialize Plan
              </button>
            </div>
          </div>
        </div>
