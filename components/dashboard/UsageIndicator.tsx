...
export function UsageIndicator({ type }: UsageIndicatorProps) {
  ...
  const fetchUsage = useCallback(async () => {
    ...
    if (type === 'documents') {
      ...
      const maxMap = { chat: 10, quizzes: 3, notes: 5, flashcards: 3 };
  ...
  return (
    <div className={`inline-flex items-center gap-3 px-4 py-2 rounded-xl border transition-all ${isNearLimit ? 'bg-red-500/5 border-red-500/20 text-red-600' : 'bg-orange-500/5 border-orange-500/10 text-orange-600'}`}>
      <div className="flex flex-col">
         ...
         <div className="h-1 w-24 bg-background/50 rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all duration-500 ${isNearLimit ? 'bg-red-500' : 'bg-orange-600'}`} 
              style={{ width: `${pct}%` }} 
            />
         </div>
      </div>
